import { 
  Controller, Get, Post, Body, Patch, Param, Query, Delete, ParseUUIDPipe,
  BadRequestException, 
...
import { Public } from 'src/auth/decorators/public.decorator'; // Ajusta según tu auth
...
async findAll(
  ...
  @ApiOperation({ summary: 'Actualizar el stock de una variante específica' })
  @Patch('variant/:idVariant/stock')
  updateStock(
    @Param('idVariant', ParseUUIDPipe) idVariant: string,
    @Body('stock') stock: number,
  ) {
    return this.productService.updateVariantStock(idVariant, stock);
  }
